//
//  LMTakuSplashDelegate.m
//  LitemobSDK
//
//  Taku/AnyThink 开屏广告代理实现
//
//  Created by Neko on 2026/01/28.
//

#import "LMTakuSplashDelegate.h"
#import "../Base/LMTakuAdapterCommonHeader.h"
#import "../Base/LMTakuBaseAdapter.h"
#import <AnyThinkSDK/AnyThinkSDK.h>
#import <Foundation/Foundation.h>
#import <LitemobSDK/LMSplashAd.h>

@implementation LMTakuSplashDelegate

#pragma mark - LMSplashAdDelegate

/// 广告加载成功
/// @param splashAd 开屏广告实例
- (void)lm_splashAdDidLoad:(LMSplashAd *)splashAd {
    LMTakuLog(@"Splash", @"广告加载成功: %@", splashAd);

    // 通过 LitemobSDK 获取广告价格（用于 C2S 竞价）
    // 注意：LitemobSDK 的 getEcpm 返回单位已经是"分"，直接使用
    NSString *priceStr = [splashAd getEcpm];

    // 使用统一的 C2S 竞价信息生成方法
    NSDictionary *infoDic = [LMTakuBaseAdapter getC2SInfo:priceStr];

    // 通知 AnyThink SDK 开屏广告加载成功，并传递 C2S 价格信息
    // 注意：开屏广告使用 atOnSplashAdLoadedExtra: 方法
    if (self.adStatusBridge && [self.adStatusBridge respondsToSelector:@selector(atOnSplashAdLoadedExtra:)]) {
        LMTakuLog(@"Splash", @"调用 atOnSplashAdLoadedExtra:, infoDic = %@", infoDic);
        [self.adStatusBridge atOnSplashAdLoadedExtra:infoDic];
    } else {
        LMTakuLog(@"Splash", @"⚠️ adStatusBridge 为空或未实现 atOnSplashAdLoadedExtra:");
    }
}

/// 广告加载失败
/// @param splashAd 开屏广告实例
/// @param error 错误信息
- (void)lm_splashAd:(LMSplashAd *)splashAd didFailWithError:(NSError *)error {
    LMTakuLog(@"Splash", @"广告加载失败: %@, error = %@", splashAd, error);
    // 通知 AnyThink SDK 广告加载失败
    if (self.adStatusBridge && [self.adStatusBridge respondsToSelector:@selector(atOnAdLoadFailed:adExtra:)]) {
        LMTakuLog(@"Splash", @"调用 atOnAdLoadFailed:, error = %@", error);
        [self.adStatusBridge atOnAdLoadFailed:error adExtra:nil];
    } else {
        LMTakuLog(@"Splash", @"⚠️ adStatusBridge 为空或未实现 atOnAdLoadFailed:adExtra:");
    }
}

/// 广告即将展示
/// @param splashAd 开屏广告实例
- (void)lm_splashAdWillVisible:(LMSplashAd *)splashAd {
    LMTakuLog(@"Splash", @"广告即将展示: %@", splashAd);
    // 通知 AnyThink SDK 广告展示成功
    // 注意：根据 AnyThink 官方文档，参数应该传 nil，而不是空字典
    if (self.adStatusBridge && [self.adStatusBridge respondsToSelector:@selector(atOnAdShow:)]) {
        LMTakuLog(@"Splash", @"调用 atOnAdShow:");
        [self.adStatusBridge atOnAdShow:nil];
    } else {
        LMTakuLog(@"Splash", @"⚠️ adStatusBridge 为空或未实现 atOnAdShow:");
    }
}

/// 广告被点击
/// @param splashAd 开屏广告实例
- (void)lm_splashAdDidClick:(LMSplashAd *)splashAd {
    LMTakuLog(@"Splash", @"广告被点击: %@", splashAd);
    // 通知 AnyThink SDK 广告被点击
    // 注意：根据 AnyThink 官方文档，参数应该传 nil，而不是空字典
    if (self.adStatusBridge && [self.adStatusBridge respondsToSelector:@selector(atOnAdClick:)]) {
        LMTakuLog(@"Splash", @"调用 atOnAdClick:");
        [self.adStatusBridge atOnAdClick:nil];
    } else {
        LMTakuLog(@"Splash", @"⚠️ adStatusBridge 为空或未实现 atOnAdClick:");
    }
}

/// 广告已关闭
/// @param splashAd 开屏广告实例
- (void)lm_splashAdDidClose:(LMSplashAd *)splashAd {
    LMTakuLog(@"Splash", @"广告已关闭: %@", splashAd);
    // 通知 AnyThink SDK 广告已关闭
    // 注意：根据 AnyThink 官方文档，参数应该传 nil，而不是空字典
    if (self.adStatusBridge && [self.adStatusBridge respondsToSelector:@selector(atOnAdClosed:)]) {
        LMTakuLog(@"Splash", @"调用 atOnAdClosed:");
        [self.adStatusBridge atOnAdClosed:nil];
    } else {
        LMTakuLog(@"Splash", @"⚠️ adStatusBridge 为空或未实现 atOnAdClosed:");
    }
}

// 释放资源
- (void)dealloc {
    self.adStatusBridge = nil;
    LMTakuLog(@"Splash", @"LMTakuSplashDelegate dealloc");
}

@end
